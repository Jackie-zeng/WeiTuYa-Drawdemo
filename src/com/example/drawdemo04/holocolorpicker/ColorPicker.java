...
public class ColorPicker extends View {
	...
	private int calculateColor(float angle) {
		...
		int c0 = COLORS[i];
		int c1 = COLORS[i + 1];
		int a = ave(Color.alpha(c0), Color.alpha(c1), p); //Round[(Color.alpha(c1)-Color.alpha(c0))*p]+Color.alpha(c0)
	...
	private float colorToAngle(int color) {
		...
		return (float) Math.toRadians(-colors[0]);  //colors[0] Й«Па
	}
	
	@Override
	public boolean onTouchEvent(MotionEvent event) {
		getParent().requestDisallowInterceptTouchEvent(true);  //don't interrupt
		...
		case MotionEvent.ACTION_DOWN:
			...
			// If user did not press pointer or center, report event not handled
				...
				getParent().requestDisallowInterceptTouchEvent(false);   //can interrupt
				return false;
			}
			break;
